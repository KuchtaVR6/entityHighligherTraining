...
class MaskedWeightedLossModel(nn.Module):
    def __init__(self, base_model, class_weights, dropout_prob=0.1):
        ...
        self.loss_fn = nn.CrossEntropyLoss(weight=torch.tensor(class_weights, dtype=torch.float), reduction="none")
    ...
    def forward(self, input_ids, attention_mask, labels, loss_mask=None, **kwargs):
        ...
        if loss_mask is not None:
            masked_loss = (loss_mask.view(-1) * self.calculate_weighted_loss(logits, labels)).mean()
        else:
